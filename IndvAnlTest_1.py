...
class Test:
    ...
    def txndf(self,category):
        if category=='ATM':
            atm_df=pd.DataFrame(self.txn_df[self.txn_df['Txn Category']=='ATM'])
            atm_df.reset_index(drop=True, inplace=True)
            atm_df=atm_df.to_html()
            print(atm_df)
        if category=='NEFT':
            neft_df=pd.DataFrame(self.txn_df[self.txn_df['Txn Category']=='NEFT'])
            neft_df.reset_index(drop=True, inplace=True)
            neft_df=neft_df.to_html()            
            print(neft_df)
        if category=='IMPS':
            imps_df=pd.DataFrame(self.txn_df[self.txn_df['Txn Category']=='IMPS'])
            imps_df.reset_index(drop=True, inplace=True)
            imps_df=imps_df.to_html()            
            print(imps_df)
        if category=='POS':
            pos_df=pd.DataFrame(self.txn_df[self.txn_df['Txn Category']=='POS'])
            pos_df.reset_index(drop=True, inplace=True)
            pos_df=pos_df.to_html()            

            print(pos_df)
        if category=='UPI':
            upi_df=pd.DataFrame(self.txn_df[self.txn_df['Txn Category']=='UPI'])
            upi_df.reset_index(drop=True, inplace=True)
            upi_df=upi_df.to_html()            
            print(upi_df)
        if category=='TPT':
            tpt_df=pd.DataFrame(self.txn_df[self.txn_df['Txn Category']=='TPT'])
            tpt_df.reset_index(drop=True, inplace=True)
            tpt_df=tpt_df.to_html()            
            print(tpt_df)
        if category=='INFT':
            inft_df=pd.DataFrame(self.txn_df[self.txn_df['Txn Category']=='INFT'])
            inft_df.reset_index(drop=True, inplace=True)
            inft_df=inft_df.to_html()            
            print(inft_df)
        if category=='ICICI Direct':
            idir_df=pd.DataFrame(self.txn_df[self.txn_df['Txn Category']=='ICICI DIRECT'])
            idir_df.reset_index(drop=True, inplace=True)
            idir_df=idir_df.to_html()            
            print(idir_df)
        
    def costdf(self):
        self.cost_df.drop(drop=True, inplace = True)

        self.cost_df=self.cost_df.to_html()
        print(self.cost_df)

    def balintdf(self):
        self.balintcombo_df.drop(drop=True, inplace=True)

        self.balintcombo_df=self.balintcombo_df.to_html()
